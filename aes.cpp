...
const uint8_t Aes::T_S_Box[16][16] = {
...
const uint8_t Aes::T_S_Box_Inv[16][16] = {
...
const uint8_t Aes::T_Mix_Col_E[16][16] = {
...
const uint8_t Aes::T_Mix_Col_L[16][16] = {
...
const uint8_t Aes::T_Const_Mix[4][4] = {
    {0x02,0x03,0x01,0x01},
    {0x01,0x02,0x03,0x01},
    {0x01,0x01,0x02,0x03},
    {0x03,0x01,0x01,0x02},
};
const uint8_t Aes::T_Const_Mix_Inv[4][4] = {
    {0x0E,0x0B,0x0D,0x09},
    {0x09,0x0E,0x0B,0x0D},
    {0x0D,0x09,0x0E,0x0B},
    {0x0B,0x0D,0x09,0x0E},
...
void Aes::_key_Expansion()
{
    ...
    {
    case keylen_128: ke_r_cnt = 10; break;
    case keylen_192: ke_r_cnt = 8; break;
...
array<uint8_t,4> Aes::_make_temp_word(array<uint8_t, 4> w, uint8_t RCon)
{
    ...
    array<uint8_t,4> res_word;
    for(int i=0;i<4;++i)
    {
        res_word[i] = T_S_Box[temp[i]>>4&0x0f][temp[i]&0x0f];
    }
    res_word[0] ^= RCon;
    return res_word;
}
...
array2d<uint8_t,4,4> Aes::_shift_rows(array2d<uint8_t,4,4> src_state, T sel)
{
    ...
    for(int i=0;i<4;++i)
    {
        for(int j=0;j<4;++j)
        {
            res_state[i][sel(j,i)%4] = src_state[i][j];
        }
    }
    return res_state;
}
